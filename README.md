...
- **Full fine-tuning extension**: a from-scratch JAX/Flax reimplementation
  ...
  GPU but ran ~8x faster than PyTorch on TPU (XLA-compiled: 27,006 vs.
  3,418 tok/s DDP, 3,219 tok/s FSDP); FSDP-style sharding cut peak memory
  ~4.4x versus DDP (26.8GB → 6.2GB on TPU). See `training/jax_v_pytorch/`.
...
JAX is roughly on par with FSDP on GPU, but ~8x faster than either PyTorch
baseline on TPU once XLA compiles the training step — and running the
...
- `curation/` — LLM-based dataset scoring, duplicate detection/filtering,
  and schema auto-detection (`preprocess.py`) for raw uploads
- `training/` — LoRA/QLoRA and full fine-tuning, auto-QLoRA decision
  (`finetune.py`), the forecasting engine and stop/continue decision
  logic (`forecasting.py`, `decision_engine.py`), and the JAX/Flax vs.
  PyTorch DDP/FSDP benchmark (`jax_v_pytorch/`)
- `evaluation/` — generation + LLM-as-judge comparison of fine-tuned vs.
  base model outputs
...
- `results/` — exported reports, run configs, and benchmark results
- `notebooks/` — exploratory research, including the forecasting-method
