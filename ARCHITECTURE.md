...
4. **Define split.** What percent of the dataset goes to train vs. validation.
5. **Curation, yes or no.** User decides whether to run LLM curation at all.
   6. If yes, choose threshold (Weak / Medium / Strong presets, or custom).
7. **Curation report.** Per-example scores and reasons, kept/rejected counts.
   7.1 Remove specific samples manually based on the report.
8. **Fine-tune.**
9. **Choose base model.** For example Qwen2.5-1.5B-Instruct, or others.
...
14. **Review curve and problems after training.** Full diagnostic pass
    once training completes.
15. **Test and compare.** LLM-as-judge (Sonnet/Haiku) compares base vs.
...
Training: 170s, 4.73GB peak GPU memory, loss 2.45 to 1.93.
...
JAX matched PyTorch/XLA FSDP throughput on GPU, but ran about 8x faster
on TPU (XLA-compiled) than PyTorch/XLA DDP (27,006 vs. 3,418 tok/s), and
the same JAX code is itself 63x faster on TPU than on the 2xT4 GPU pair.

FSDP-style sharding cut peak memory about 4.4x versus DDP (26.8GB to
6.2GB per chip on a v6e-4 TPU).
...
Fine-Tuning" method is marked "JAX, research build" for that reason;
LoRA/QLoRA is the only method that runs live through the UI today). See
